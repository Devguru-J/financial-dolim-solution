...
### 2026-04-16 업데이트
- ✅ **Excel parity 하네스 4-스크립트 파이프라인** — BNK 워크북 AppleScript 자동 구동 → 엔진 결과 diff
  - `extract-bnk-operating-lease-fixture.ts` — 단일 시나리오 추출 (Es1 셀 직접 쓰기)
  - `run-bnk-sweep.ts` — 배치 하네스 (JSON 시나리오 array → chunk 기반 실행 + checkpoint resume)
  - `run-bnk-parity-diff.ts` — sweep 결과 ↔ 엔진 순수함수 diff (허용오차: pmt ±100원, rate ±0.0001, residual ±1000원)
  - `generate-bnk-scenarios.ts` — 파서 기반 다축 시나리오 생성 (term × residualMode × ownership × vehiclesPerBrand)
- ✅ **150-시나리오 다축 스윕 100% match** — 15 수입 브랜드 × 2 vehicles × 5 terms (12/24/36/48/60m) × high mode
- ✅ **BNK 엔진 7개 버그 수정** (하네스가 surface):
  - B173 슈퍼카 +0.6% (페라리/롤스로이스/람보르기니/맥라렌/애스턴마틴 company+gap>0)
  - B174 애스턴마틴 company -0.3%
  - B176 랜드로버+재규어/랜드로버-도이치 -1.1%
  - C175 테슬라 -1.1% / C177 폴스타·마세라티·벤틀리 -1.1%
  - CR provider dealer gate — 5개 딜러 한정 (람보르기니 6.5M 잔가 격차 해결)
  - EV/HEV 자동 취득세 감면 — 전기 -140만원, HEV -40만원
  - defaultAcquisitionTaxRate — 화물 5%, 승용 일괄 7% (EV cc=0 포함)
- ✅ **BNK 파서 3개 수정**:
  - RVs gradeColEnd 98→100 (BNK_30 매트릭스 누락)
  - ecoType/ecoTaxEligible 필드 추가 (CDB col 14/29)
  - parseCond dealerName = col 3 VLOOKUP key (혼다-비제휴 매핑 수정)
- ✅ **크로스 금융사 매칭 개선** — BRAND_ALIASES 확장 (ASTON_MARTIN↔애스턴마틴, ROLLS_ROYCE↔롤스로이스, MCLAREN↔맥라렌). 현재 MG→BNK 81.2% 매칭 (워크북 기준 최대)
- ✅ **미취급 금융사 graceful 처리** — 차량이 특정 금융사에 없으면 해당 금융사 결과 자동 숨김 (에러 박스 대신). `useMultiQuote`에 `notAvailable` 플래그 + `NOT_AVAILABLE_PATTERNS` 감지
- ⚠️ **발견: BNK B71 (고잔가 토글) 무의미** — Excel B50=MAX(G101:G107)가 항상 boosted max rate 사용. standard/high 구분이 B50 경로에서 무의미. 엔진 residualMode="standard"는 Excel parity target 없음
...
bun test             # 패리티 테스트 (158개: MG 46 + BNK 17 + vehicleKey 68 + cross-lender audit 11 + BNK 유닛 11 + BNK 통합 5)
bun run typecheck    # TS 타입 검사
bun run db:push      # DB 스키마 마이그레이션

# Excel parity 하네스 (BNK — Excel 열려있어야 함)
bun run scripts/generate-bnk-scenarios.ts --output=scenarios.json --vehiclesPerBrand=2 --terms=60
bun run scripts/run-bnk-sweep.ts --scenarios=scenarios.json --output=sweep.json --chunkSize=15
bun run scripts/run-bnk-parity-diff.ts --scenarios=scenarios.json --sweep=sweep.json --output=diff.json
```
